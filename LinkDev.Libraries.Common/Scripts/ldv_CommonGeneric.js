...
function LoadRichEditor(fieldName)
{
	/// <summary>
	...
	{
        parent.CKEDITOR.replace('ckWysiwyg');

        var editor = parent.CKEDITOR.dom.element.get('ckWysiwyg').getEditor();
		...
		if (editor)
		{
			...
			var setEditorData = function ()
			{
                ...
                parent.CKEDITOR.instances.ckWysiwyg.setData(Xrm.Page.getAttribute(fieldName).getValue());
...
function ApplyActionToField(name, fetchFunction, action, value)
{
	...
	while (control = fetchFunction(indexedName))
    {
        if (!name || !control)
        {
            console.error('Could not find control "' + name + '" to apply action "' + action + '".');
        }
...
function SetupFieldNameAutoComplete(entityNameFieldName, fieldNameFieldName, maxResults, callback, isUseCache)
{
	/// <summary>
	...
	///     Author: Ahmed el-Sawalhy
	/// </summary>
	if (!GetFieldValue(entityNameFieldName))
	{
		console.error('SetupFieldNameAutoComplete: entity name is empty.');
		return;
	}

	var entityMetadata = EntityMetadata[entityNameFieldName];

	var loadCallback = function()
	{
		entityMetadata = EntityMetadata[entityNameFieldName];
	...
	{
		LoadEntityMetadata(entityNameFieldName, loadCallback);
	}
}
...
function ExecuteOnNotDirty(callback)
{
	...
	setTimeout(function()
	{
		...
		if (!IsFormDirty())
		{
			// reload page after a little delay
			setTimeout(function()
			{
				if (callback)
				{
...
function LoadEntityMetadata(entityNameFieldName, callback, errorCallback)
{
    /// <summary>
	/// Internal use only!
    /// </summary>
	...
	if (!entityName)
	{
		console.error('LoadEntityMetadata: entity name is empty.');
		return;
	}
	...
	RetrieveEntityMetadata(entityName, null, null, ['LogicalName', 'DisplayName'], null,
		function(metadata)
		{
			...
			EntityMetadata[entityNameFieldName] = metadata;
